...
def create_raid_disks(mount_point, mount_point_owner, mount_point_group, mount_point_mode,
                      ...
                      snapshots, disk_type, disk_piops, encrypted, encryption_passwd, dm_name, hvm)
  ...
  if not creating_from_snapshot
    ...
    execute "creating raid device" do
      ...
      command "mdadm --create /dev/#{raid_dev} --level=#{level} --chunk=#{chunk_size} --raid-devices=#{devices.size} #{devices_string}"
    end
